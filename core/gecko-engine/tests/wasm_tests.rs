use gecko_engine::sandbox::engine::{HostImports, ScriptExecutor, ExtensionCallback};
...
fn test_wasm_simple_execution() {
    ...
    assert_eq!(result.output, serde_json::json!({"result": 42, "status": "success"}));
}
...
fn test_wasm_host_bindings() {
    ...
    let result = executor.evaluate(code, Uuid::new_v4(), &HostImports::default(), None, Some(cb));
...
fn test_wasm_epoch_timeout() {
    ...
    let result = executor.evaluate(code, Uuid::new_v4(), &HostImports::default(), Some(50), None);
    
    assert!(!result.success, "Infinite loop should have timed out and failed");
}
