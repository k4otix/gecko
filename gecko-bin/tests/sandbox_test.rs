...
"#;
    ...
    db.apply_schema(core_schema).await.expect("Failed to apply core schema");
    
    // 4. Sync the bundle
    let _sync_result = sync_bundle(&mut db, &manifest).await.expect("Failed to sync bundle");
    ...
    if answer.is_document_stream() {
        ...
        if let Some(Ok(doc)) = stream.next().await {
            ...
            if let Some(code) = json.as_object().and_then(|m| m.get("code")).and_then(|v| v.as_str()) {
                code_block = code.to_string();
            }
        }
    }
    
    assert!(!code_block.is_empty(), "Failed to fetch code block from database");
